...
def _materialize_file_backed_param(module, param_name, param, device, model_dtype=None):
    """Materialize one NoHostMMap parameter without calling ``meta.to``.

    NoHostMMap deliberately represents large safetensors values as meta tensors
    carrying a file slice.  This guard is installed after that loader in some
    ComfyUI startup orders, so its old ``param.data.to(...)`` path would try to
    copy a meta tensor and fail with ``Cannot copy out of meta tensor``.
    """
    if not getattr(param, "is_meta", False):
        return False
    try:
        info = getattr(param.untyped_storage(), "_comfy_tensor_file_slice", None)
    except RuntimeError:
        info = None
    if info is None:
        return False
    ...
    # Read using the on-disk dtype first.  A model dtype conversion, when
    # requested by ComfyUI, happens after the bounded device transfer and thus
    # never creates a host-sized staging allocation.
    loaded = _file_slice_tensor_to_device(param, torch.device(device), param.dtype)
    if model_dtype is not None and loaded.dtype != model_dtype:
        loaded = loaded.to(dtype=model_dtype)
    # Replacing ``Parameter.data`` fails after a DynamicVRAM unload/reload when
    # the cached meta parameter and the newly materialized CUDA tensor have
    # different tensor implementations.  ComfyUI's setter replaces the
    # Parameter object itself and is the same mechanism used by NoHostMMap's
    # primary loader.
    import comfy.utils
    ...
    for n, m in _iter_named_modules_nonrecursive(self.model):
        ...
        if default and default_device is not None:
            for param_name, param in params.items():
                model_dtype = getattr(m, param_name + "_comfy_model_dtype", None)
                if _materialize_file_backed_param(
                    m, param_name, param, default_device, model_dtype
                ):
                    continue
