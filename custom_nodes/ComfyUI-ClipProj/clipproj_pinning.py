...
def _cuda_parameter_bytes(module):
    """Return bytes still owned by CUDA parameters/buffers in *module*.

    DynamicVRAM's ``model_unload`` only releases its VBAR and host-pin state.
    The no-host-mmap resident path materialises the actual parameters in the
    module tree, so those allocations must be inspected independently of
    ComfyUI's bookkeeping.  Counting storage pointers once avoids reporting
    tied weights twice (Qwen's embedding/lm-head is commonly shared).
    """
    if module is None:
        return 0
    total = 0
    ...
    for tensor in tensors:
        try:
            ...
            # A meta or custom quantized tensor can reject one of the storage
            # accessors; it must not prevent the remaining model from moving.
            continue
    return total


def _empty_cuda_allocators():
    """Synchronize and empty every CUDA allocator, not only the current card."""
    try:
        ...
        for index in range(torch.cuda.device_count()):
            try:
                ...
                logging.debug("[ClipProj] CUDA cache clear on %d failed: %s", index, error)
    except Exception:
        pass


def _move_resident_model_to_cpu(patcher, label=""):
    """Physically move a resident encoder (including MP tail layers) to CPU.

    ``ModelPatcherDynamic.unpatch_model`` intentionally leaves materialised
    parameters in place because its normal contract is paging, not a complete
    model eviction.  ClipProj's resident pre-DiT boundary is different: the
    whole encoder must leave both GPUs before a long H3 activation is created.
    ``Module._apply`` is used as a fallback so a partially file-backed model
    can skip meta tensors instead of failing with ``Cannot copy out of meta``.
    """
    model = getattr(patcher, "model", None)
    if model is None or not hasattr(model, "to"):
        return False
    before = _cuda_parameter_bytes(model)
    try:
        with torch.no_grad():
            model.to(device=torch.device("cpu"))
    except Exception as error:
        # A mixed meta/materialised tree cannot use Module.to().  Preserve the
        # meta leaves and move every real tensor one by one; this also handles
        # a future ComfyUI loader that leaves a small auxiliary tensor lazy.
        logging.debug("[ClipProj] bulk CPU move for %s fell back to _apply: %s", label, error)
        try:
            def to_cpu(tensor):
            ...
            logging.error("[ClipProj] physical CPU offload failed for %s: %s", label, fallback_error)
            return False

    _empty_cuda_allocators()
    remaining = _cuda_parameter_bytes(model)
    if remaining:
        logging.error(
            "[ClipProj] physical CPU offload incomplete for %s: %.1f MiB remains on CUDA",
            label,
            remaining / 2**20,
        )
        return False
    try:
        ...
        pass
    patcher._clipproj_cpu_offloaded = True
    logging.info(
        "[ClipProj] %s physically moved to CPU (%.2f GB; resident MP owners included)",
        label or "encoder",
        before / 2**30,
    )
    return True
...
def unload_patcher(patcher, label="", force_cache=True):
    ...
    Args:
        ...
        force_cache (bool): whether to request a PromptExecutor cache reset
            while emptying CUDA caches.  Pre-DiT release passes ``False`` so
            conditioning/node results remain reusable on the next request.
    ...
    gc.collect()
    mm.soft_empty_cache(force=force_cache)
    if freed and label:
        logging.info("[ClipProj] %s unloaded (%.2f GB freed)", label, size)
    return size if freed else 0.0


def release_role(role, key=None, force_cache=True):
    ...
    if key is not None and prev_key == key:
        return
    unload_patcher(ref(), label, force_cache=force_cache)
    _ACTIVE.pop(role, None)



def release_device(carte, garde=None, force_cache=True):
    ...
    Args:
        ...
        force_cache (bool): whether to force an executor cache reset.
    ...
    for role in list(_ACTIVE):
        ...
        if len(e) > 3 and e[3] == carte:
            ...
            total += unload_patcher(e[1](), e[2], force_cache=force_cache)
            _ACTIVE.pop(role, None)
    return total


def release_all(force_cache=True):
    ...
    for role in list(_ACTIVE.keys()):
        ...
        got = unload_patcher(ref(), label, force_cache=force_cache)
        if got > 0:
            count += 1
            total += got
        _ACTIVE.pop(role, None)
