...
def _cached_shift(values, device):
    """Cache tiny device-local shift tensors used by every Q/K block."""
    key = (torch.device(device), tuple(values))
    result = _SHIFT_CACHE.get(key)
    if result is None:
        result = torch.tensor(values, device=device, dtype=torch.uint8)
        _SHIFT_CACHE[key] = result
    return result
...
def dequantize_blocks_Q6_K(blocks, block_size, type_size, dtype=None):
    ...
    ql = ql.reshape((n_blocks, -1, 1, 64)) >> _cached_shift((0, 4), d.device).reshape((1, 1, 2, 1))
    ql = (ql & 0x0F).reshape((n_blocks, -1, 32))
    qh = qh.reshape((n_blocks, -1, 1, 32)) >> _cached_shift((0, 2, 4, 6), d.device).reshape((1, 1, 4, 1))
...
def dequantize_blocks_Q5_K(blocks, block_size, type_size, dtype=None):
    ...
    ql = qs.reshape((n_blocks, -1, 1, 32)) >> _cached_shift((0, 4), d.device).reshape((1, 1, 2, 1))
    qh = qh.reshape((n_blocks, -1, 1, 32)) >> _cached_shift(tuple(range(8)), d.device).reshape((1, 1, 8, 1))
    ql = (ql & 0x0F).reshape((n_blocks, -1, 32))
...
def dequantize_blocks_Q4_K(blocks, block_size, type_size, dtype=None):
    ...
    qs = qs.reshape((n_blocks, -1, 1, 32)) >> _cached_shift((0, 4), d.device).reshape((1, 1, 2, 1))
...
def dequantize_blocks_Q3_K(blocks, block_size, type_size, dtype=None):
    ...
    lscales = lscales.reshape((n_blocks, 1, 8)) >> _cached_shift((0, 4), d.device).reshape((1, 2, 1))
    lscales = lscales.reshape((n_blocks, 16))
    hscales = hscales.reshape((n_blocks, 1, 4)) >> _cached_shift((0, 2, 4, 6), d.device).reshape((1, 4, 1))
    ...
    ql = qs.reshape((n_blocks, -1, 1, 32)) >> _cached_shift((0, 2, 4, 6), d.device).reshape((1, 1, 4, 1))
    qh = hmask.reshape(n_blocks, -1, 1, 32) >> _cached_shift(tuple(range(8)), d.device).reshape((1, 1, 8, 1))
...
def dequantize_blocks_Q2_K(blocks, block_size, type_size, dtype=None):
    ...
    shift = _cached_shift((0, 2, 4, 6), d.device).reshape((1, 1, 4, 1))
