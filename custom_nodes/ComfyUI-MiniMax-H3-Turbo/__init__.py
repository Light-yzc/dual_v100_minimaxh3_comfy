...
class MiniMaxH3TurboLoRA:
    ...
    def apply_lora(self, model, lora_name, strength, low_vram=False):
        ...
        lora = _canonicalize_lora(lora)
        dm = model.model.diffusion_model
        pruned = getattr(dm, "use_adaln_curves", False)
        modules = _lora_module_names(lora)
        if not modules:
            raise ValueError(
                f"LoRA has no supported A/B factors: {lora_name}"
            )
        ...
        print(f"[MiniMaxH3TurboLoRA] {'pruned' if pruned else 'full'} base [{mode}]: "
              ...
              f"modules ({raw_lora_key_count} raw keys -> {len(lora)} canonical), "
              f"{detail}{extra} | model={type(new_model.model).__name__} "
