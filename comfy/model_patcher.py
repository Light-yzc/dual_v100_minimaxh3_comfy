    ...
    def load(self, device_to=None, lowvram_model_memory=0, force_patch_weights=False, full_load=False, dirty=False):
        ...
        with self.use_ejected():
                ...
                *_, module_mem, n, m, params = x
                ...
                if hasattr(m, "comfy_cast_weights"):
                    ...
                    for param in params:
                        ...
                        storage_slice = getattr(
                            weight.untyped_storage(), "_comfy_tensor_file_slice", None
                        ) if getattr(weight, "is_meta", False) else None
                        if storage_slice is not None:
                            # NoHostMMap keeps non-cast parameters as meta tensors
                            # with a file-offset descriptor. Materialize this one
                            # tensor directly on the target device before any dtype
                            # conversion; calling meta.to(...) would have no data
                            # to copy and would defeat the no-host-mmap path.
                            from custom_nodes.NoHostMMap.safetensors import (
                                _file_slice_tensor_to_device,
                            )
                            casted_weight = _file_slice_tensor_to_device(
                                weight, torch.device(device_to), weight.dtype
                            )
                            if model_dtype is not None and casted_weight.dtype != model_dtype:
                                casted_weight = casted_weight.to(dtype=model_dtype)
                        else:
            ...
            for key, buf in self.model.named_buffers(recurse=True):
                ...
                storage_slice = getattr(
                    buf.untyped_storage(), "_comfy_tensor_file_slice", None
                ) if getattr(buf, "is_meta", False) else None
                if storage_slice is not None:
                    from custom_nodes.NoHostMMap.safetensors import (
                        _file_slice_tensor_to_device,
                    )
                    casted_buf = _file_slice_tensor_to_device(
                        buf, torch.device(device_to), buf.dtype
                    )
                    if model_dtype is not None and casted_buf.dtype != model_dtype:
                        casted_buf = casted_buf.to(dtype=model_dtype)
                else:
