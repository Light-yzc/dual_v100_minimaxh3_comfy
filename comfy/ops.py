...
def materialize_meta_param(s, param_keys):
    for param_key in param_keys:
        ...
        if param is not None and getattr(param, "is_meta", False):
            # NoHostMMap and the GGUF dynamic loader attach a file slice to a
            # meta tensor.  Replacing that tensor with zeros here would both
            # discard the disk location and potentially change a quantized
            # storage tensor into a full BF16/FP16 tensor.  The gathered copy
            # path below knows how to read this slice directly into VBAR.
            storage_tensor = getattr(param, "_qdata", param)
            storage = getattr(storage_tensor, "untyped_storage", lambda: None)()
            if storage is not None and (
                getattr(storage, "_comfy_tensor_file_slice", None) is not None
                or getattr(storage, "_comfy_tensor_mmap_refs", None) is not None
            ):
                continue
            setattr(s, param_key, torch.nn.Parameter(torch.zeros(param.shape, dtype=param.dtype), requires_grad=param.requires_grad))


def ensure_vbar_capacity(s):
    """Repair a stale VBAR slot before interpreting it as a tensor buffer.

    Dynamic patchers allocate ``m._v`` while walking the model's state dict.
    A file-backed/no-mmap load can replace a parameter after that walk (for
    example, an INT8 storage tensor can become its BF16 model tensor).  In that
    case the old slot is smaller than the tensor that the transfer path is
    about to interpret, and the failure appears as an opaque "Buffer too
    small" exception.  Reallocate only the affected layer; the VBAR is a
    virtual-address arena and this does not create a host-side checkpoint copy.
    """
    allocation = getattr(s, "_v", None)
    if allocation is None:
        return

    geometry = comfy.memory_management.tensors_to_geometries([s.weight, s.bias])
    required_size = comfy.memory_management.vram_aligned_size(geometry)
    current_size = int(allocation[2])
    if required_size <= current_size:
        return

    module_name = getattr(s, "seed_key", "<unnamed>")
    logging.warning(
        "AIMDO VBAR resize for %s: allocation=%d bytes required=%d; "
        "weight_shape=%s weight_dtype=%s model_dtype=%s bias_shape=%s",
        module_name,
        current_size,
        required_size,
        tuple(s.weight.shape) if s.weight is not None else None,
        getattr(s.weight, "dtype", None),
        getattr(s.weight, "_model_dtype", None),
        tuple(s.bias.shape) if s.bias is not None else None,
    )
    comfy_aimdo.model_vbar.vbar_unpin(allocation)
    vbar = allocation[0]
    s._v = vbar.alloc(required_size)
    s._v_signature = None
    for attr in ("_v_weight", "_v_bias"):
        if hasattr(s, attr):
            delattr(s, attr)
