...
def load_safetensors(ckpt):
    no_host_mmap = os.environ.get("H3_NO_HOST_MMAP", "1").lower() not in {"0", "false", "no"}
    if no_host_mmap:
        # Read only the 8-byte length and JSON header.  Tensor payloads stay on
        # disk; meta tensors below carry their offsets into TensorFileSlice.
        ...
        try:
            ...
            for name, info in header.items():
                ...
                if start == end:
                    sd[name] = torch.empty(info["shape"], dtype=_TYPES[info["dtype"]], device="meta")
                    continue
                tensor = torch.empty(info["shape"], dtype=_TYPES[info["dtype"]], device="meta")
                ...
                # Keep the ordinary descriptor open until all state-dict
                # tensors have been discarded.  No mmap object is retained.
                setattr(storage, "_comfy_tensor_file_refs", (f,))
                sd[name] = tensor
