...
class MiniMaxH3ImageToVideo(io.ComfyNode):
    ...
    def execute(cls, clip, vae, prompt, width, height, length,
        ...
        if first_frame is not None:
            # Both keyframes must use the same geometry transform.  Stretching
            # the first frame while cover-cropping the last frame makes a
            # repeated image change shape between t=0 and t=end.
